...
class SitaConnector {
  ...
  final Map<String, String> _credentials = {
    ...
    'tbPassword': 'PILOT123',
    '__VIEWSTATE':
        '/wEPDwUKMTU0MTkzOTcyNGQYAQUeX19Db250cm9sc1JlcXVpcmVQb3N0QmFja0tleV9fFgEFDEltYWdlQnV0dG9uMVf4peHA9d+SZhEg3eHjzBu0/C2z',
    '__VIEWSTATEGENERATOR': 'CA0B0334',
    '__EVENTVALIDATION':
        '/wEdAAaht1oI1IQNClsWsvomI4+HpdJy3Je1PsStj1TIeUIVoYSOl4uuYcHEPP0KEAhfCSGoV7n24SKj5sCn/fdhk5Hd6ZACrx5RZnllKSerU+IuKqKeKEbp39eHc9mbdvkCgxDIJE2G6ueYhlurUuNCVTZt7Zu14Q==',
    'ImageButton1.x': '55',
    'ImageButton1.y': '16'
  };
  ...
  Future<String> init() async {
    ...
    try {
      initialResponse = await this._client.get(this._landingUrl);
    } catch (Exception, e) {
    ...
    http.Response loginResponse = await this._client.post(this._landingUrl,
        headers: {"Cookie": sessionCookie}, body: this._credentials);

    if (loginResponse.statusCode != 302) return 'Login to SITA website failed!';
  ...
  Future<List<Map<String, String>>> getFlightPlanList() async {
    ...
    http.Response flightPlansResponse = await this._client.post(
    ...
    for (var match in flightDataMatches) {
      String url =
          'https://www.flightops.sita.aero/FlightPlanning/PlanDetail.aspx?PlanId=' +
              match.group(1);
      if (match.group(2) != 'OMA9999' && match.group(2) != 'OMATEST') {
  ...
  Future<String> getFlightPlanText(String flightReference) async {
    http.Response flightPlanResponse = await _client
        .get(_buildFlightUrl(flightReference), headers: {"Cookie": _cookies});
